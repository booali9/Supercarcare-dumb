...
import { getTowTruckService } from "./graphql/queries";
import { updateTowTruckService } from "./graphql/mutations";
const client = generateClient();
export default function TowTruckServiceUpdateForm(props) {
  const {
    id: idProp,
    towTruckService: towTruckServiceModelProp,
    onSuccess,
    onError,
    onSubmit,
    onValidate,
  ...
  const resetStateValues = () => {
    const cleanValues = towTruckServiceRecord
      ? { ...initialValues, ...towTruckServiceRecord }
      : initialValues;
  ...
  const [towTruckServiceRecord, setTowTruckServiceRecord] = React.useState(
    towTruckServiceModelProp
  );
  React.useEffect(() => {
      ...
      const record = idProp
        ? (
            await client.graphql({
              query: getTowTruckService.replaceAll("__typename", ""),
              variables: { id: idProp },
            })
          )?.data?.getTowTruckService
        : towTruckServiceModelProp;
      setTowTruckServiceRecord(record);
    };
    queryData();
  }, [idProp, towTruckServiceModelProp]);
  React.useEffect(resetStateValues, [towTruckServiceRecord]);
  const validations = {
    storeId: [],
    available: [],
    contactName: [],
    ...
    <Grid
      ...
      onSubmit={async (event) => {
        ...
        try {
          ...
          await client.graphql({
            query: updateTowTruckService.replaceAll("__typename", ""),
            variables: {
              input: {
                id: towTruckServiceRecord.id,
                ...modelFields,
              },
            },
          });
      ...
      {...getOverrideProps(overrides, "TowTruckServiceUpdateForm")}
      {...rest}
    >
      <TextField
        label="Store id"
      ...
      >
        <Button
          ...
          isDisabled={!(idProp || towTruckServiceModelProp)}

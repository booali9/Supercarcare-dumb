...
import { Menu, X, ChevronDown, User, Shield, Truck, Star, CreditCard, Facebook, Twitter, Instagram, Youtube } from "lucide-react";
...
const Header = () => {
  ...
  return (
    <header
      className={`fixed w-full z-50 backdrop-blur-xl transition-all duration-300 ${scrolled ? 'shadow-lg bg-black/95 py-1' : ''}`}
      style={{
        background: `
    ...
    >
      ...
      />      <div className={`relative px-6 transition-all duration-300 ${scrolled ? 'py-1' : 'py-3'}`}>
        <div className="flex flex-col items-center w-full">
          <div className="flex flex-col items-center w-full">
            <a href="/" className="flex flex-col items-center group mb-2" onClick={() => handleNavClick("/")}> 
              ...
              <div className="mt-2 text-center">
                <h1 className={`font-black text-white tracking-wider transition-all duration-300 ${scrolled ? 'text-lg' : 'text-xl'}`} style={{ fontFamily: 'Inter' }}>SUPERCAR CARE CENTER</h1>
                <span className="text-xs text-white/60 font-medium tracking-wide block">Premium Auto Care</span>
              </div>
            </a>
          </div>
          {/* Centered navigation and social icons below logo */}
      ...
      {showResearchDropdown && (
        <div className="hidden md:block absolute top-full left-1/2 transform -translate-x-1/2 bg-black/95 backdrop-blur-md rounded-md shadow-2xl mt-2 p-4 z-50 border border-white/10 w-80">
